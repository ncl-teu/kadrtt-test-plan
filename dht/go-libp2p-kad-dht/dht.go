...
import (
	...
	//kb "github.com/ncl-teu/go-libp2p-kadrtt-kbucket"
...
type IpfsDHT struct {
	...
	beta       int // The number of peers closest to a target_/:;;l:;/. that must have responded for a query path to terminate
...
func makeDHT(ctx context.Context, h host.Host, cfg config) (*IpfsDHT, error) {
	...
	//Added by Kanemimtsu
	/*if dht.isKadRTT {
		rt, err := makeKadRTTRoutingTable(dht, cfg, 2*maxLastSuccessfulOutboundThreshold)
		if err != nil {
			return nil, fmt.Errorf("failed to construct KadRTT routing table,err=%s", err)
		}
		dht.routingTable_rtt = rt
		//dht.routingTable = rt
...
//Added by Kanemitsu
/*func makeKadRTTRoutingTable(dht *IpfsDHT, cfg config, maxLastSuccessfulOutboundThreshold time.Duration) (*kb.KadRTTRT, error) {
